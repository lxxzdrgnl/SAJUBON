...
2. **탭 구성 — 총 7~10개**:
   - **앵커 탭 3개(항상 포함, 순서 고정)**:
     ① 종합 케미 — 두 사람 전체 관계의 결론 헤드라인 (total_score 기반, 강점 중심)
   ...
   - **가변 탭 4~6개 (신호 보고 선택)**:
     ...
     소통·신뢰, 오행 균형, 용신 보완(yongsin_help), 갈등·마찰(clash_pairs가 있을 때만 1개)
...
3. **관점 — 두 사람의 '특징'을 긍정적으로**:
   ...
   - 갈등·마찰은 한 탭에서만 균형 있게 다루고, 그 탭조차 "이렇게 풀면 강점이 된다"로 마무리하세요. 리포트 전체가 부정적으로 읽히면 안 됩니다.
