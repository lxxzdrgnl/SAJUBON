...
8. **인라인 차트 마커**: 탭 본문(content) 안에 아래 마커를 삽입해 시각 자료를 보여주세요.
   ...
   - 마커는 **그 차트를 설명하는 본문 문단 바로 앞 줄에 단독으로** 놓으세요(차트가 설명 위에 먼저 보이도록). 즉 마커 한 줄 → **빈 줄** → 해당 내용 문단 순서.
   - **마지막 문단(③ 조언)에는 차트 마커를 절대 넣지 마세요.** 조언은 텍스트로만 마무리하고, 차트는 그 앞의 근거 문단(②)에 배치하세요. 차트를 본문 맨 끝(조언 뒤)에 붙이지 마세요.
