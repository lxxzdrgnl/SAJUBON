...
8. **인라인 차트 마커**: 탭 본문(content) 안에 아래 마커를 삽입해 시각 자료를 보여주세요.
   ...
   - **반드시 아래 예시의 문단 배치를 그대로 따르세요** (한 탭 content 예시 — 차트가 비유 다음·근거 앞 중간에 오고, 조언은 차트 없이 끝맺음):
     ```
     이용재님과 유다연님은 마치 단단한 땅에 뿌리내린 나무와 그 옆을 스치는 바람처럼, 서로에게 자극과 성장을 안기는 관계입니다.

     [[chart:compat_wuxing_a]]

     위 오행 분포에서 이용재님은 토와 화가 강해 안정과 열정을 함께 지닙니다. 반면 유다연님은 목이 강해 성장과 진취성이 돋보여, 두 사람은 서로 부족한 기운을 채워줍니다.

     서로의 다름을 인정하고 각자의 강점을 존중한다면, 두 분은 한층 깊이 소통하며 시너지를 낼 수 있습니다.
     ```
   - **위 예시는 '차트가 들어가는 탭'에서의 배치 방법을 보여줄 뿐, 모든 탭에 차트를 넣으라는 뜻이 절대 아닙니다.** 차트는 위 규칙대로 관련 내용이 있는 탭에만(각 마커 한 번씩) 넣고, 어울리지 않는 탭은 차트 없이 본문만 쓰세요. 차트 없는 탭이 여러 개 있는 게 정상입니다.
