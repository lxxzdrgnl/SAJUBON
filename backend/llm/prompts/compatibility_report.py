...
8. **인라인 차트 마커**: 탭 본문(content) 안에 아래 마커를 삽입해 시각 자료를 보여주세요.
   ...
   - **차트를 풍부하게 쓰세요 — 9개 마커 중 최소 6개 이상**을 서로 다른 탭에 분산 배치하세요. 특히 **A·B 각자의 팔자(compat_palja_a/b)와 오행(compat_wuxing_a/b)은 반드시 포함**하고, 십성·강약도 어울리는 탭에 넣으세요. 차트가 2~3개만 쓰이면 부족합니다.
   - 각 마커는 전체 리포트에서 **한 번씩만** 사용하고, 가장 잘 어울리는 탭 본문에 배치하세요(한 탭에 몰지 말고 여러 탭에 흩뿌리기).
