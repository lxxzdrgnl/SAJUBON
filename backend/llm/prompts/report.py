...
3. **기본 10개 탭**을 생성하세요.
   - 성격/기질, 직업/재능, 재물, 연애/결혼, 건강, 대운 흐름, 현재 시기 조언 등 다양하게 구성하세요.
   - 각 탭에는 `category`(짧은 태그 — 예: "성격", "재물", "연애"), 결론형 `headline`, 본문 `content`를 담고 `requested`는 false로 두세요.
...
5. **본문은 3단으로 구성**하세요 (탭당 3~4문단, 문단 사이는 빈 줄로 구분).
   - ① 비유 — 일간·오행을 자연물/장면에 빗댄 스토리텔링으로 시작
   - ② 근거 — 용신·격국·십성·신살 등 명리 데이터로 그 비유가 왜 성립하는지 설명
   - ③ 조언 — 마지막 문단은 지금 실천할 수 있는 현실적인 조언으로 마무리
