...
7. **원국 차트 마커 — 어울리는 차트를 가장 맞는 탭 본문에 끼워 넣으세요.**
   ...
   - **반드시 아래 예시의 문단 배치를 그대로 따르세요** (한 탭 content 예시 — 차트가 비유 다음·근거 앞 중간에 오고, 조언은 차트 없이 끝맺음):
     ...
     ```
   - **위 예시는 '차트가 들어가는 탭'에서의 배치 방법을 보여줄 뿐, 모든 탭에 차트를 넣으라는 뜻이 절대 아닙니다.** 차트는 위 규칙대로 관련 탭에만(각 마커 전체 리포트에서 한 번씩) 넣고, 차트가 어울리지 않는 탭은 차트 없이 본문만 쓰세요. 차트 없는 탭이 여러 개 있는 게 정상입니다.
