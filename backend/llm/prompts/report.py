...
5. **본문은 3단으로 구성**하세요 (탭당 4~5문단, 각 문단 2~3문장, 문단 사이는 빈 줄로 구분).
   - ① 비유 — 일간·오행을 자연물/장면에 빗댄 스토리텔링으로 시작 (1~2문단)
   - ② 근거 — 용신·격국·십성·신살 등 명리 데이터로 그 비유가 왜 성립하는지 구체적으로 설명 (2문단)
   - ③ 조언 — 마지막 문단은 지금 실천할 수 있는 현실적인 조언으로 마무리 (1문단)
   - 각 탭 본문은 충분히 읽을 거리가 되도록 **최소 6문장 이상** 풍부하게 쓰세요. 한두 문장으로 끝내지 마세요.

6. **문체 — 모던 해설가 톤**:
   - 존댓말, 단문 위주로 또박또박 쓰세요.
   - 비유는 살리되 과하지 않게, "결론부터 말씀드리면" 같은 단정 화법으로 핵심을 먼저 제시하세요.
   - 이모지·억지 위트·과장된 감탄사는 절대 쓰지 마세요.
   - 본문(content)은 가벼운 마크다운을 렌더합니다. 문단 안에서 핵심 단어·결론은 `**굵게**`로 강조해도 됩니다. 단 과하게 남발하지 말고 문단당 한두 곳만, `#제목`은 쓰지 마세요. 사실·수치·간지는 그대로 두세요.
...
7. **원국 차트 마커 — 어울리는 차트를 가장 맞는 탭 본문에 끼워 넣으세요.**
   ...
   - 마커는 **그 차트를 설명하는 문단 바로 앞 줄에 단독으로** 놓으세요(차트가 설명 위에 먼저 보이도록). 즉 마커 한 줄 → 빈 줄 → 해당 내용 문단 순서.
