...
interface Props {
  ...
  submitLabel?: string
  busy?: boolean
  /** true 시 이름 필드가 선택 입력이 됨 (파트너 첨부 등). false/미지정이면 필수. */
...
export default function BirthInputForm({
  ...
  const nameError = nameOptional ? null : (name.trim() ? null : t('errorName'))
    ...
    <div className="flex flex-col gap-4 rounded-lg border-2 border-ink bg-surface p-4 shadow-brutal">
      ...
      <label className="block">
        <span className="mb-1.5 block text-xs font-extrabold">
          {t('name')}
          {!nameOptional && <em className="not-italic text-orange"> *</em>}
        </span>
        <input
          value={name}
          maxLength={20}
