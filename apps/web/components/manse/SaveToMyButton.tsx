...
export default function SaveToMyButton({ birthInput, isLoggedIn, googleLoginUrl }: Props) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="mt-2 flex flex-col gap-2 rounded-2xl border-2 border-ink bg-surface p-4 shadow-[4px_4px_0_#1A1A1A]">
        <p className="text-[13px] text-text-sub">{t('saveToMyLoginHint')}</p>
        <a
          href={googleLoginUrl}
          className="block w-full rounded-xl border-2 border-ink bg-surface py-3 text-center text-sm font-extrabold shadow-[2px_2px_0_#1A1A1A] transition-opacity hover:opacity-80"
        >
          {t('saveToMyLoginLink')}
        </a>
      </div>
  ...
  return (
    <button
      ...
      className={
        'mt-2 block w-full rounded-xl border-2 border-ink py-3 text-center text-sm font-extrabold shadow-[2px_2px_0_#1A1A1A] transition-opacity disabled:opacity-60 ' +
        (state === 'done'
