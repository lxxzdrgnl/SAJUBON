...
- **redis 다운 → enqueue 실패**: `create_job_and_enqueue`가 enqueue를 try/except로 감싸 실패 시 `HTTPException(503, "잠시 후 다시 시도")` 발생. job 행은 flush만 됐고 commit 전이라 세션 종료 시 롤백 → **유령 pending job 없음**. 프론트는 503을 에러로 표시.
